...
# Main Query, add types here and provide resolvers
class Query(graphene.ObjectType):
  ...
  def resolve_tracks(self, info, search=None):
    if search:
      ...
      return Track.objects.filter(filter)  # __gt __startswith __contains __exact __iexact
    return Track.objects.all()

  def resolve_likes(self, info):
    return Like.objects.all()

# Mutations (remember to add to main Mutation)
